...
- `API_KEY`: Must be at least 8 characters long in production environments to avoid security warnings. Setting the `API_KEY` to an empty string will explicitly disable API authentication, triggering a severe security warning in the logs. If completely omitted in Docker deployments, `docker-compose.yml` ensures a safe, non-empty default fallback string is used rather than unintentionally disabling authentication.
...
- **Timeouts/Intervals**: Limits are strictly enforced. `AI_TIMEOUT_MS` is bounded between `1000` and `60000`. `STATUS_POLL_INTERVAL_MS` ensures a minimum of `1000` ms to prevent event loop starvation; if a non-numeric string or a value below 1000ms is provided, it logs a warning and falls back to a safe 60000ms.
